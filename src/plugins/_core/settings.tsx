...
function PluginCard({
    ...
    React.useEffect(() => {
        async function getStatus() {
            ...
            console.log('fetched status', currentPluginStatus);
            setPluginStatus(currentPluginStatus);
            console.log('set status', currentPluginStatus);
        }

        if(!pluginStatus) {
            console.log('plugin status doesnt exist');
            getStatus();
        }
    }, []);

    console.log('before ret status', pluginStatus);

    return (
        <div className='settings-text-colour'>
            <div className='settings-flex-row'>
                <h1>{plugin.name}</h1>
                <span className='settings-text-secondary-colour'>v{plugin.version}</span>
                <input
                    type="checkbox"
                    ...
                    onClick={(async (e) => {
                        ...
                        if(newStatus === PluginStatus.CRASHED) {
                            console.error('Plugin crashed upon settings page start', plugin.name, newStatus);
                        }
            ...
            <div>
                {plugin.authors && plugin.authors.length > 0 && <span>by {plugin.authors?.map((authors) => authors).join(', ')}</span>}
            </div>
        </div>
    );
}
...
export default definePlugin({
        ...
        return (
            <div style={{
                ...
                <div style={{
                    ...
                    onClick={(() => {
                        // open settings page somehow
                        settingsContext.setSettingsShown(!settingsContext.settingsShown);
                        console.log('on click called', settingsContext.settingsShown);
                    })}
                >
                    <button
                        type='button'
    ...
    start: () => {
        ...
        patcher.after(obj, key, (args, result, ctx) => {
            console.log('patched zne called', args, result, ctx);
            return (
                <SettingsProvider>
                    {result}
                </SettingsProvider>
