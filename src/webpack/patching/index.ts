...
import { pluginPatches } from '@/plugins/api/setupPlugins';
import { allowForSelf } from './regexWrappers';
            ...
            for(const patch of pluginPatch.patches) {
                    ...
                    if(typeof matcher === 'string') {
                        ...
                        if(match) {
                            ...
                            for(let i = 0; i < match.length; i++) {
                                ...
                                newReplace = newReplace.replace(new RegExp('\\$' + i, 'g'), match[i]);
                            }
                ...
                for(const replacement of patch.replacements) {
                    try {
                        ...
                        logger.log('patches owned by ', pluginPatch.owner);
                        logger.log('find/replacement', patch.find, replacement);
                        if(typeof replacement.replace === 'function') {
                            newSource = replacement.replace(moduleSource);
                        } else {
